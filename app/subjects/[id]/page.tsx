...
export default function SubjectDetailPage() {
  ...
  if (!subject) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/subjects" className="text-sm text-primary hover:underline mt-2 inline-block">Back to subjects</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <Link href="/subjects" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="rounded-xl border bg-card p-6">
        <div className="flex items-start gap-4">
          ...
          >
            <IconComp className="h-6 w-6" style={{ color: subject.color }} />
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="font-display text-2xl md:text-3xl font-bold tracking-tight">{subject.name}</h1>
            {subject.description && (
              <p className="text-sm text-muted-foreground mt-1">{subject.description}</p>
            )}
            <div className="flex items-center gap-4 mt-3">
              <span className="text-xs text-muted-foreground">{topics.length} Topics</span>
              <span className="text-xs text-muted-foreground">{progress.total} Tasks</span>
              <span className="text-xs text-muted-foreground">
                {subject.lastStudiedAt ? `Last studied ${formatRelativeTime(subject.lastStudiedAt)}` : 'Not started yet'}
              </span>
            </div>
      ...
      <div>
        ...
        {topics.length === 0 ? (
          <div className="rounded-xl border border-dashed bg-card p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground/70 mt-1 mb-4">Add your first topic to start studying.</p>
            <Button onClick={() => setAddOpen(true)} size="sm" variant="outline">
              <Plus className="h-4 w-4 mr-1" />
              Add Topic
            </Button>
          ...
          <div className="space-y-2">
            {topics.map((topic, idx) => {
              const tp = getTopicProgress(data.tasks, topic.id);
              const noteCount = data.notes.filter((n) => n.topicId === topic.id).length;
              const taskCount = data.tasks.filter((t) => t.topicId === topic.id).length;
              return (
                <div
                  key={topic.id}
                ...
                >
                  ...
                  <Link href={`/subjects/${subjectId}?topic=${topic.id}`} className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        <FileText className="h-3 w-3" /> {noteCount}
                      </span>
                      <span className="text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-medium w-9 text-right">{tp.pct}%</span>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onSelect={() => handleEditTopic(topic)}>
                          <Pencil className="mr-2 h-3.5 w-3.5" />
                          Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => setDeleteTopicId(topic.id)} className="text-destructive">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          ...
          <TopicForm
            initial={{ subjectId }}
            onSaved={() => { setAddOpen(false); toast({ title: 'Topic added' }); }}
        ...
        <DialogContent>
          ...
          {editTopic && (
            <TopicForm
              ...
              onSaved={() => { setEditOpen(false); toast({ title: 'Topic updated' }); }}
      ...
      <AlertDialog open={!!deleteTopicId} onOpenChange={(v) => !v && setDeleteTopicId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this topic?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the topic, its tasks, and its notes. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
